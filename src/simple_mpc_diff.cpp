...
int main( )
{
  ...
  /*
    ...
    */ 
        ...
        rf  << x << y << v << phi << aplha_w; // << a << deltarate;
        rfN << x << y << v << phi;

        const int N  = 10; //was 5
        const int Ni = 5;
        const double Ts = 0.1;
		...
		ocp.subjectTo( -M_PI <= aplha_w <= M_PI );
        //ocp.subjectTo( 0.05*0.05 <= w <= 1.0*1.0 );
        ...
        if (mpc.exportCode( "simple_mpc_diff_export" ) != SUCCESSFUL_RETURN)
                exit( EXIT_FAILURE );
