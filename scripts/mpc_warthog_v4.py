...
def make_twist_msg(accel, acc_omega, goalData, warn_w, yaw_meas, ppx):
    global vel_up
    global vel_down
    global w_up
    global latest_yaw
    dt_in = 0.1
    cmd = Twist()
    print(f"ppx in make twist msg: {ppx}, current_robot_position: {robot_state.get_current_pos_meas()}")
    if not goalData[0]:
        ...
        if dToGoal < defs.DIST_TO_GOAL_STOP: #was .1
            ...
            if (robot_state.get_current_pos_meas() != ppx):
                cmd.linear.x = 0
                cmd.angular.z = 0
                print("Goal Reached")
                vel_down = defs.MIN_TARGET_SPEED
...
def mpc_node():
    ...
    prune_done_ = 1
    index_pruning = index_no_pruning
    ...
    while not rospy.is_shutdown():
        ...
        if prune_done:
            ...
            print(f"goal: {goal}, target_ind_moce: {target_ind_move}, len(cx): {len(cx)}, cx-offset: {len(cx)-offset_stop}")
            goalData = utils.check_goal(robot_state.get_current_pos_meas(), goal, target_ind_move, len(cx)-offset_stop)
            print(f"Goal Data: {goalData}")
            #print(dt_cmd)
            #warn_w = False
            ...
            # print(warn_w)
            cmd_command = make_twist_msg(ai, wi, goalData, warn_w, robot_state.yaw, ppx[-1])
...
if __name__ == '__main__':
    #args = rospy.myargv(argv=sys.argv)
    #print(args)
    # mpc_node()    #DELETE FOR UI Usage

    # if rospy.has_param('ui_stat'):
    #     param_value = rospy.get_param('/ui_stat')
    #     # print(f"Param value from UI is: {param_value}")

    #     while (not param_value):
    #         param_value = rospy.get_param('/ui_stat')
    #         print(f"Param value from UI is: {param_value}")
    #     print(f"Param value from UI after loop is: {param_value}")
        mpc_node()
