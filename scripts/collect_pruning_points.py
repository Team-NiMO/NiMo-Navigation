...
if __name__ == "__main__":
	...
	while True:
            x = input('Save_point?')
            if x=='y':
               c = np.loadtxt('pruning_points.csv',delimiter=',')
               print(c)
               if(len(c) == 0):
                 print(gr.robot_position)
                 np.savetxt('pruning_points.csv',[gr.robot_position],delimiter=',')
               else:
                 print('position ',gr.robot_position)
                 print('here')
                 np.savetxt('pruning_points.csv',np.vstack([c,gr.robot_position]),delimiter=',')
